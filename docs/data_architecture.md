...
- That red-email path (`scripts/sentinel-v5/checkin-lib.sh`) no longer discards the Resend response; it now saves one content-free receipt per send attempt (timestamp, job, reason code, status, HTTP code, Resend message id — never a message body or secret) to `~/.streetlight/mail-red/receipts.log`, matching the same-day receipt allowlist above. The per-item six-hour cooldown is unchanged and still shared with `sl-error-stream-health`.
