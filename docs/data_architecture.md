...
- A `launchd` job on the operator's own machine (`com.streetlight.ui-sentry`, `scripts/ui-sentry/`) drives production in a real browser daily and, as a named exception to the operator-monitoring boundary above, may additionally call `/api/chat` with synthetic fixture content — capped at 8 live turns per run, enforced at the request boundary, ≤56 live turns/week (see the 2026-09-26 cadence correction below).
...
**2026-09-26 — UI sentry: daily cadence corrected, DEGRADED now reaches jesse@ as red, and its red mail keeps a receipt:**

- The job has run daily since 2026-08-23 (commit `8bc68a9`, #30); this document and the 2026-08-07 ADR still said Mon/Wed/Fri and computed the turn-budget ceiling from a 3-runs/week schedule. Both are corrected above and in the ADR (≤56 live turns/week ceiling, not ≤24).
- Root-caused and fixed a gap where a DEGRADED run (e.g. Turnstile blocking most, not all, live-chat turns) never reached a human: the sentinel-v5 `sl-ui-sentry` check-in went red only on a process crash, and `sl-ui-sentry-live-chat` only on 10-day staleness — neither reads the run's own verdict. `sl-ui-sentry` now also reports red (`reason_code: degraded`) whenever `last-run.json`'s `overallLevel` is `DEGRADED` or `FAIL`, so it reaches the existing direct red-email path (added by #43) like any other red.
- That red-email path (`scripts/sentinel-v5/checkin-lib.sh`) no longer discards the Resend response; it now saves one content-free receipt per send attempt (`timestamp`/`job`/`httpStatus`/`resendId`, plus this path's own fixed `reason` field — never a message body or secret) to `~/.streetlight/mail-red/receipts.log`. Field names and shape now actually match the same-day receipt allowlist above field-for-field (a cross-vendor review, 2026-09-26, caught that the first cut only claimed to match — different field names, no UUID validation on the id, and any 2xx started the cooldown even without one; all three are fixed, and the send itself is now locked per item with an Idempotency-Key — see the ADR amendment below). The per-item six-hour cooldown is unchanged and still shared with `sl-error-stream-health`.
- The same function's write to the retired sentinel-v5 spool (`~/.blt-sentinel/spool`, no consumer since 2026-09-24) is removed; nothing downstream of it read that write.
...
**2026-09-26 — cross-vendor review follow-up: a real turn-classification bug, a fail-open verdict check, mail-send locking, and the monitor-pass wiring:**

- Root-caused and fixed a real bug behind `client_blocked`, separate from the pacing theory two entries up: the app's send-failure notice is one fixed string in a plain `useState`, so two client-blocked turns in a row leave the SAME DOM node untouched (React bails out on an unchanged value) — a turn that actually went on to send could be misread as blocked from a prior turn's leftover notice. Fixed by watching the submit control's own busy state instead of notice text.
- The DEGRADED-is-red verdict check (added a few hours earlier, same day) failed open: missing `jq`, an unreadable/malformed `last-run.json`, a missing `overallLevel`, or state left over from a previous invocation all defaulted to green. Now requires a recognized level AND a `startedAt` at or after this invocation's own start, or reports red `state_unverifiable`.
- The red-mail cooldown is now locked per item (reusing the same-day alert-receipts work's `mail-lock.py`) with the reservation written before sending and a per-item Idempotency-Key header, closing a concurrent-double-send and a timeout-then-resend gap. Receipt field names and the id-validation rule now actually match the same-day receipt allowlist above.
...
**2026-09-26 — third cross-vendor review: the 3/6 cause retracted to unknown, the mail path simplified, two more fixes:**

- Retracted the previous entry's "classification bug made the 3/6 result" explanation: the run log shows only three `/api/chat` POSTs were ever made for a six-turn attempt, and a misattribution bug can only explain a response being read wrong, never a request never being sent. The real cause is unknown pending a live run instrumented to capture it.
- The mail-send locking design above (a cross-invocation pending key with bounded retries) was itself producing new bugs — a retry could rebuild the payload under the same key, an unverifiable 2xx was treated as a definite rejection, and a Doppler-status read lived inside a subshell and never reached the caller. Simplified per coordinator decision: send at most once per invocation, one immediate in-process retry of a network error with the identical payload/key, and a four-outcome model (confirmed/rejected/pre_send_failure/uncertain) recorded directly in the receipt. An unresolved 2xx or network failure now starts the cooldown anyway — a rare missed duplicate is acceptable, a real duplicate email is not.
- The turn-classification fix's response/body reads still had no bound of their own and could hang past the turn's configured deadline; both are now raced against it. A repeated block that also resolves synchronously hit neither existing fast-detection path; an in-page MutationObserver on the submit button now catches it regardless of external polling cadence.
- The DEGRADED-is-red verdict check's jq read did not reject a file containing more than one valid JSON value (a fresh PASS followed by a stray `{}` still read green) and did not validate field types. Fixed with `jq -s`/`length != 1`, strict type checks, and a real invocation-identity match between the producer and the emitter.
- The back-navigation fix's `back_forward` signal is not proof of a bfcache resume (the spec defines it as any history-traversal navigation). Switched to `pageshow`'s own `persisted` boolean, the actual documented signal.
